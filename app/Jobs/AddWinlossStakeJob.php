    ...
    public function handle(): void
    {
        ...
        if ($winloss_day) {
            if ($this->jenis == 'deduct') {
                $winloss_day->increment('stake', $this->amount);
            } else if ($this->jenis == 'settle') {
                $winloss_day->increment('winloss', $this->amount);
            }
        } else {
        ...
        if ($winloss_month) {
            if ($this->jenis == 'deduct') {
                $winloss_month->increment('stake', $this->amount);
            } else if ($this->jenis == 'settle') {
                $winloss_month->increment('winloss', $this->amount);
            }
        } else {
        ...
        if ($winloss_year) {
            if ($this->jenis == 'deduct') {
                $winloss_year->increment('stake', $this->amount);
            } else if ($this->jenis == 'settle') {
                $winloss_year->increment('winloss', $this->amount);
            }
        } else {
            winlossYear::create([
                'username' => $this->username,
